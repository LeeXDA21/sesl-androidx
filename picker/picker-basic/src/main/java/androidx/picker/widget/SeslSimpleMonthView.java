...
public class SeslSimpleMonthView extends View {
    ...
    int mLastAccessibilityFocusedView = View.NO_ID;
    private int mMiniDayNumberTextSize;
    private int mMode = DATE_MODE_NONE;
    int mMonth;
    private int mNormalTextColor;
    int mNumCells = DEFAULT_NUM_DAYS;
    ...
    boolean mIsFirstMonth = false;
    private boolean mIsHcfEnabled = false;
    boolean mIsLastMonth = false;
    boolean mIsLeapMonth = false;
    boolean mIsLunar = false;
    boolean mIsNextMonthLeap = false;
    boolean mIsPrevMonthLeap = false;
    private boolean mIsRTL;
    private boolean mLockAccessibilityDelegate;
    boolean mLostAccessibilityFocus = false;
    ...
    SeslSimpleMonthView(Context context, AttributeSet attrs, int defStyle) {
        ...
        context.getTheme().resolveAttribute(androidx.appcompat.R.attr.colorPrimary, outValue, true);//sesl6
    ...
    int getDaysInMonthLunar(int month, int year, boolean isLeapMonth) {
    ...
    private void drawDays(Canvas var1) {
        ...
        for(var2 = var21; var2 <= this.mNumCells; ++var2) {
            ...
            if (this.mMode == DATE_MODE_NONE && var2 == var17) {
                var26.setColor(this.mSelectedDayNumberTextColor);
            }
        ...
        if (!this.mIsLastMonth) {
            ...
            while(true) {
                ...
                if (this.mMode != DATE_MODE_NONE && var17 == this.mNumCells + 1) {
        ...
        if (var4 > 0 && !this.mIsFirstMonth) {
            ...
            for(var11 = 0; var11 < var4; ++var11) {
                ...
                if (this.mMode != DATE_MODE_NONE && var31 == 0) {
    ...
    int getDayFromLocation(float x, float y) {
    ...
    void onDeactivatedDayClick(int year, int month, int day, boolean isPrevMonth) {
        if (!mIsLunar) {
            mTempDate.clear();
