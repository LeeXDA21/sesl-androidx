...
class SeslColorSwatchView extends View {
    ...
    private static final float SWATCH_ITEM_ROW = 10f;
    private static final float SWATCH_ITEM_COLUMN = 11f;
    ...
    private final Point mCursorIndex;
    private Rect mCursorRect;
    OnColorSwatchChangedListener mListener;
    Resources mResources;
    private Rect mShadowRect;
    Paint mStrokePaint;
    private final RectF mSwatchRect;
    private final RectF mSwatchRectBackground;
    SeslColorSwatchViewTouchHelper mTouchHelper;
    Paint shadow;

    private float[] corners;
    float mSwatchItemHeight;
    float mSwatchItemWidth;

    private int ROUNDED_CORNER_RADIUS_IN_Px = 0;
    private int currentCursorColor;
    int mSelectedVirtualViewId = NO_ID;

    private boolean mFromUser = false;
    private boolean mIsColorInSwatch = true;

    int[][] mColorSwatch = new int[][] {
    ...
    StringBuilder[][] mColorSwatchDescription
            = (StringBuilder[][]) Array.newInstance(StringBuilder.class,
            (int)SWATCH_ITEM_COLUMN,  (int)SWATCH_ITEM_ROW);
    ...
    public SeslColorSwatchView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        mStartMargin = mResources.getDimensionPixelSize(R.dimen.sesl_swatch_rect_starting);
        mTopMargin = this.mResources.getDimensionPixelSize(R.dimen.sesl_swatch_rect_top);

        mSwatchRect = new RectF(
                mStartMargin +  SWATCH_ITEM_SIZE_ROUNDING_VALUE,
                mTopMargin + SWATCH_ITEM_SIZE_ROUNDING_VALUE,
                mStartMargin + mResources.getDimensionPixelSize(R.dimen.sesl_color_picker_oneui_3_color_swatch_view_width) + SWATCH_ITEM_SIZE_ROUNDING_VALUE,
                mTopMargin +  mResources.getDimensionPixelSize(
                        R.dimen.sesl_color_picker_oneui_3_color_swatch_view_height) + SWATCH_ITEM_SIZE_ROUNDING_VALUE);

        mSwatchRectBackground
                = new RectF(mStartMargin,
                0.0f,
        ...
        for (int i = 0; i < SWATCH_ITEM_COLUMN; i++) {
            for (int j = 0; j < SWATCH_ITEM_ROW; j++) {
                ...
                if (i == 0 && j == 0) {
                    ...
                    Path path = new Path();
                    path.addRoundRect((int) (
                                    mStartMargin +  i * mSwatchItemWidth + SWATCH_ITEM_SIZE_ROUNDING_VALUE),
                            mTopMargin + (int) (j * mSwatchItemHeight + SWATCH_ITEM_SIZE_ROUNDING_VALUE),
                            mStartMargin + (int) (mSwatchItemWidth * (i + 1) + SWATCH_ITEM_SIZE_ROUNDING_VALUE),
                            mTopMargin + (int) (mSwatchItemHeight * (j + 1)+ SWATCH_ITEM_SIZE_ROUNDING_VALUE),
                            corners,
                            Path.Direction.CW);
                    ...
                    Path path = new Path();
                    path.addRoundRect(
                            mStartMargin + (int) (i * mSwatchItemWidth + SWATCH_ITEM_SIZE_ROUNDING_VALUE),
                            mTopMargin + (int) (j * mSwatchItemHeight + SWATCH_ITEM_SIZE_ROUNDING_VALUE),
                            mStartMargin + (int) (mSwatchItemWidth * (i + 1) + SWATCH_ITEM_SIZE_ROUNDING_VALUE),
                            mTopMargin + (int) (mSwatchItemHeight * (j + 1) + SWATCH_ITEM_SIZE_ROUNDING_VALUE),
                            corners,
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (setCursorIndexAt(event.getX()- mStartMargin, event.getY() - mTopMargin)
                || !mIsColorInSwatch) {
    ...
    StringBuilder getColorSwatchDescriptionAt(int color) {
        ...
        if (mColorSwatchDescription[cursorIndex.x][cursorIndex.y]
            ...
            return mTouchHelper.getItemDescription(cursorIndex.x
                    + (int)(cursorIndex.y * SWATCH_ITEM_COLUMN));
        }
    ...
    private void setSelectedVirtualViewId() {
        mSelectedVirtualViewId = (int)(mCursorIndex.y * SWATCH_ITEM_COLUMN)
                + mCursorIndex.x;
    }
    ...
    private void setCursorRect(Rect rect) {
        rect.set(
                mStartMargin + (int) (((mCursorIndex.x - 0.05d) * mSwatchItemWidth) + 4.5d),
                mTopMargin + (int) (((mCursorIndex.y - 0.05d) * mSwatchItemHeight) + 4.5d),
                mStartMargin + (int) (((mCursorIndex.x + 1 + 0.05d) * mSwatchItemWidth) + 4.5d),
                mTopMargin + (int) (((mCursorIndex.y + 1 + 0.05d) * mSwatchItemHeight) + 4.5d));
    }

    private void setShadowRect(Rect rect) {
        rect.set(
                mStartMargin + (int) ((mCursorIndex.x * mSwatchItemWidth) + SWATCH_ITEM_SIZE_ROUNDING_VALUE),
                mTopMargin + (int) ((mCursorIndex.y * mSwatchItemHeight) + SWATCH_ITEM_SIZE_ROUNDING_VALUE),
                mStartMargin + (int) (((mCursorIndex.x + 1 + 0.05d) * mSwatchItemWidth) + 4.5d),
                mTopMargin + (int) (((mCursorIndex.y + 1 + 0.1d) * mSwatchItemHeight) + 4.5d));
    }
    ...
    private class SeslColorSwatchViewTouchHelper extends ExploreByTouchHelper {
        ...
        private final String[][] mColorDescription = new String[][] {
                new String[] {
        ...
        private int getFocusedVirtualViewId() {
            return mVirtualCursorIndexX + (int)(mVirtualCursorIndexY * SWATCH_ITEM_COLUMN);
        }
        ...
        private void setVirtualCursorIndexAt(int id) {
            mVirtualCursorIndexX = id % (int)SWATCH_ITEM_COLUMN;
            mVirtualCursorIndexY = id / (int)SWATCH_ITEM_COLUMN;
        }

        private void setVirtualCursorRect(Rect rect) {
            rect.set(
                    mStartMargin + (int) ((mVirtualCursorIndexX * mSwatchItemWidth) + SWATCH_ITEM_SIZE_ROUNDING_VALUE),
                    mTopMargin + (int) ((mVirtualCursorIndexY * mSwatchItemHeight) + SWATCH_ITEM_SIZE_ROUNDING_VALUE),
                    mStartMargin + (int) (((mVirtualCursorIndexX + 1) * mSwatchItemWidth) + SWATCH_ITEM_SIZE_ROUNDING_VALUE),
                    mTopMargin + (int) (((mVirtualCursorIndexY + 1) * mSwatchItemHeight) + SWATCH_ITEM_SIZE_ROUNDING_VALUE));
        }

        StringBuilder getItemDescription(int id) {
